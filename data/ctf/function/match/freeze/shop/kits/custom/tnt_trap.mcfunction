...
$execute if score defense bought.trap.tnt matches 0 run function ctf:match/freeze/shop/templates/buy_item {item: frog_spawn_egg, nbt: 'item_model=tnt, entity_data={id:"minecraft:armor_stand", Marker:true, Invisible:true, Tags:["trap", "trap_display", "trap.tnt"], ArmorItems:[{}, {}, {}, {id:"minecraft:tnt"}]}', cost: $(cost), amount: 3, item_name: '$(item_name)'}

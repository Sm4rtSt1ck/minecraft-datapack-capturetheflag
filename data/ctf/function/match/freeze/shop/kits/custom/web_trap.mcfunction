$function ctf:match/freeze/shop/templates/show_item {\
    ...
    nbt: '\
        ...
        entity_data={\
            ...
            Marker:true, \
            Invisible:true, \
            Tags:["trap", "trap_display", "trap.web"], \
            ArmorItems:[{}, {}, {}, {id:"minecraft:cobweb"}]\
        }\
    '\
}
...
$execute if score web_trap bought.item matches 0 run function ctf:match/freeze/shop/templates/buy_item {\
    item: frog_spawn_egg, \
    nbt: '\
        item_model=cobweb, \
        entity_data={\

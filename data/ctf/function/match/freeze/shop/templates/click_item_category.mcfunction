...
$execute as @s[nbt=!{Inventory:[{components:{"minecraft:custom_data":{tags:["$(category)"]}}}]}] run function ctf:match/freeze/shop/templates/buy_item {item: '$(item)', cost: $(cost), amount: $(amount), item_name: '$(item_name)', nbt: "$(nbt)", tag: "$(category)"}

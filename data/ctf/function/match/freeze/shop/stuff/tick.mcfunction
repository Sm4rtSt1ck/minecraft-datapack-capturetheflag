...
execute as @s[nbt=!{Inventory:[{components:{"minecraft:custom_data":{tags:["shop", "item"]}}, Slot:22b}]}] run \
    function ctf:match/freeze/shop/stuff/custom/buy_feather_falling_boots {slot: 22, cost: 100, item_name: '{"text": "Less fall damage"}'}

execute as @s[nbt=!{Inventory:[{components:{"minecraft:custom_data":{tags:["shop", "item"]}}, Slot:24b}]}] run \
    function ctf:match/freeze/shop/templates/click_item {item: totem_of_undying, slot: 24, cost: 300, tag: "", item_name: '{"text": "Totem of undying (regen. 3.5-4 hearts)"}', amount: 1, nbt: ''}
